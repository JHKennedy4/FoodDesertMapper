...
function addPoint(store) {
    ...
    var infoTemplate = new esri.InfoTemplate("${Name}", "${Form}"),
        ...
        form = "<form><h3>Please share the available:</h3><input id='grain' type='checkbox' name='grain'><label for='grain'>Grains, Bread, Cereals</label><br /><input id='veg' type='checkbox' name='veg'><label for='veg'>Vegetables: fresh, canned, or frozen</label><br /><input id='fruit' type='checkbox' name='fruit'><label for='fruit'>Fruits: whole fruits and fruit juices</label><br /><input id='milk' type='checkbox' name='milk'><label for='milk'>Milk Products: milk, cheese, yogurt, etc.</label><br /><input id='meat' type='checkbox' name='meat'><label for='meat'>Meat, Beans, Nuts, Eggs</label><br /><input type='submit' name='Submit'></form>",
