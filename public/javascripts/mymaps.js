...
function addPoint(store) {
    ...
    var infoTemplate = new esri.InfoTemplate("${Name}", "${Form}"),
        ...
        form = "<form><h3>Here will go some info</h3><br /><input id='grain' type='checkbox' name='grain' value='true'><label for='grain'>Grains, Bread, Cereals</label><br /><input id='veg' type='checkbox' name='veg' value='true'><label for='veg'>Vegetables: fresh, canned, or frozen</label><br /><input id='fruit' type='checkbox' name='fruit' value='true'><label for='fruit'>Fruits: whole fruits and fruit juices</label><br /><input id='milk' type='checkbox' name='milk' value='true'><label for='milk'>Milk Products: milk, cheese, yogurt, etc.</label><br /><input id='meat' type='checkbox' name='meat' value='true'><label for='meat'>Meat, Beans, Nuts, Eggs</label></form>",
